...
- Band levels calibrated against the test track so each sits ~0.1–0.6 with
  headroom instead of pinning at its `min(1.0, …)` clamp.
...
- `kick_trig.threshup` (2.2) — kick sensitivity, on the *normalized* ratio.
